...
from rlkit.torch_d4rl.torch_rl_algorithm import TorchTrainer
...
class CQLTrainer(TorchTrainer):
    ...
    def _get_tensor_values(self, obs, actions, network=None):
        ...
        preds = preds.clone().view(obs.shape[0], num_repeat, 1)
        return preds
    ...
    def train_from_torch(self, batch):
        ...
        if self.num_qs == 1:
            q_new_actions = self.qf1(obs, new_obs_actions) #change
        else:
        ...
        policy_loss = (alpha*log_pi - q_new_actions.detach().clone()).mean() #change this
        ...
        if self._current_epoch < self.policy_eval_start:
            ...
            policy_loss = (policy_log_prob*log_pi - policy_log_prob).mean() #change this
        ...
        if not self.max_q_backup:
            ...
            if not self.deterministic_backup:
                target_q_values = target_q_values.clone() - alpha * new_log_pi
        ...
        ## add CQL
        random_actions_tensor = torch.FloatTensor(q2_pred.shape[0] * self.num_random, actions.shape[-1]).uniform_(-1, 1).cuda() #changed
        ...
        min_qf1_loss = min_qf1_loss.clone() - q1_pred.mean() * self.min_q_weight
        min_qf2_loss = min_qf2_loss.clone() - q2_pred.mean() * self.min_q_weight

        if self.with_lagrange:
            alpha_prime = torch.clamp(self.log_alpha_prime.exp(), min=0.0, max=1000000.0)
            min_qf1_loss = alpha_prime * (min_qf1_loss.clone() - self.target_action_gap)
            min_qf2_loss = alpha_prime * (min_qf2_loss.clone() - self.target_action_gap)
        ...
        qf1_loss = qf1_loss.clone() + min_qf1_loss
        qf2_loss = qf2_loss.clone() + min_qf2_loss

        """
        Update networks
        """
        # Update the Q-functions if
        # self.policy_optimizer.zero_grad()
        # policy_loss.backward(inputs= list(self.policy.parameters()),retain_graph=False) #error here
